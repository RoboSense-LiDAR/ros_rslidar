...
static const size_t packet_size =
        sizeof(rslidar::VelodynePacket().data);
    ...
    Input(private_nh, port)
{
    ...
    memset(&ServerAddr, 0, sizeof(ServerAddr));    // initialize to zeros
    ServerAddr.sin_family = AF_INET;
    ServerAddr.sin_port = htons(port_dest);
    ServerAddr.sin_addr.s_addr = inet_addr(send_IP);
    ...
    my_addr.sin_port = htons(port_local);
    ...
    Ret = connect(sockfd_, (sockaddr *)&ServerAddr, sizeof(ServerAddr));
    if (Ret == -1)
    {
...
int InputSocket::getPacket(rslidar::VelodynePacket *pkt, const double time_offset)
{
    double time1 = ros::Time::now().toSec();
    ///
    char * sendData = "sssssssssssssssss";
    int ret = sendto(sockfd_, sendData, strlen(sendData), 0, (sockaddr *)&ServerAddr, len);
    if (ret == -1)
    {
    ...
    {
        //std::cout << "CJF sendto  Ok！" << std::endl;
    }
    ...
    socklen_t sender_address_len = sizeof(ServerAddr);
...
*/
        ...
        //        if (-1 == ret)
        //        {
        //            ROS_INFO("sendto  failed！" );
        //        }
        //        else
        //        {
        //            //cout << "CJF sendto  Ok！" << endl;
        //        }
...
/** @brief Get one velodyne packet. */
int InputPCAP::getPacket(rslidar::VelodynePacket *pkt, const double time_offset)
{
